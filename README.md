...
開啟瀏覽器訪問: http://localhost:32111/ui
...
```bash
# 情境 A: 在主機上直接測試（非容器環境）
curl http://localhost:32110

# 情境 B: 在同主機的容器內測試（例如在 mcp-server-development-guide 容器內）
curl http://agentgateway-registration-guide:32110

# 情境 C: 從遠端主機測試（跨主機部署）
curl http://<YOUR_HOST>:32110  # 請將 <YOUR_HOST> 替換為 AgentGateway 主機的實際 IP 地址
...
- **情境 C**: 適用於從其他主機訪問 AgentGateway（需確保防火牆允許 32110 端口）
...
services:
  agentgateway-registration-guide:
    ...
    ports:
      - "32110:32110"          # MCP 代理端點
      - "0.0.0.0:32111:15000" # 管理 UI
...
| 端口 | 用途 | 訪問方式 |
|------|------|----------|
| **32110** | MCP 代理端點 | AI Client 連接此端口 |
| **32111** | Web 管理 UI | 瀏覽器訪問 http://localhost:32111/ui |
...
1. 開啟瀏覽器: http://localhost:32111/ui
...
```
AI Client → AgentGateway:32110 → MCP Server:1024
```
...
|------|------------|------------------|---------|---------|
| **場景 A** | 容器內 (同主機) | 容器內 (同主機) | 容器名稱 + 端口 | `http://agentgateway-registration-guide:32110` |
| **場景 B** | 遠端主機 | AgentGateway 主機 | IP + 端口 | `http://10.28.33.15:32110` |
| **場景 C** | 本機主機 (非容器) | 本機主機 | localhost + 端口 | `http://localhost:32110` |

**關鍵注意事項**：
- ❌ **錯誤**：如果 Client 和 AgentGateway 在同一台主機的不同容器中，不能使用 IP 地址和端口來訪問
- ✅ **正確**：在同主機容器環境中，必須使用容器名稱和端口（例如：`http://agentgateway-registration-guide:32110`）
- ✅ **正確**：如果 Client 在主機上（非容器），可以使用 `http://localhost:32110`
- ✅ **正確**：如果 Client 和 AgentGateway 在不同主機上，使用 IP 地址和端口（例如：`http://10.28.33.15:32110`）
...
uv run agentgateway_client.py test-connection \
  --url http://agentgateway-registration-guide:32110
...
python agentgateway_client.py test-connection \
  --url http://10.28.33.15:32110

# 本機測試使用 localhost
python agentgateway_client.py test-connection \
  --url http://localhost:32110
...
**測試連接**：
```
$ uv run agentgateway_client.py test-connection --url http://localhost:32110

正在連接到 http://localhost:32110/sse...
✓ 連接成功!

✓ 成功連接到 http://localhost:32110
連接已關閉
```

**列出工具**：
```
$ uv run agentgateway_client.py list-tools --url http://localhost:32110
...
**調用工具**：
```
$ uv run agentgateway_client.py call-tool --url http://localhost:32110 --tool get_greeting --args '{"name": "Alice"}'
...
```bash
# 執行完整驗證
python agentgateway_client.py validate --url http://localhost:32110
```
...
1. **選擇正確的 URL**
   - 容器內測試: `http://agentgateway-registration-guide:32110`
   - 主機上測試: `http://localhost:32110`
   - 跨主機測試: `http://<IP>:32110`
...
**基礎 URL**: `http://localhost:32110`

這是 AI Client 連接的主要端點。所有 MCP 協議請求都應發送到此端點。

```bash
# 健康檢查
curl http://localhost:32110/health
...
**症狀**：
- 瀏覽器無法開啟 http://localhost:32111/ui
- 連接被拒絕或超時
